...
func writeNFORun(cmd *cobra.Command, args []string) {
	...
	// series, err := sdb.GetSeriesEx(es.SeriesId)
	// if err != nil {
	// 	fmt.Printf("can't get series from tvdb, error: %v\n", err)
	// 	os.Exit(1)
	// }
	// if err := sdb.GetSeriesActors(series); err != nil {
	// 	fmt.Printf("can't get series actors from tvdb, error: %v\n", err)
	// 	os.Exit(1)
	// }
	// if err := sdb.GetSeriesImages(series); err != nil {
	// 	fmt.Printf("can't get series images from tvdb, error: %v\n", err)
	// 	os.Exit(1)
	// }
	// if err := sdb.GetSeriesSummary(series); err != nil {
...
func writeEpisodeNFO(epf *namer.EpisodeFile, overrideImage bool) (err error) {
	...
	// 	Plot:    ep.Overview,
	// 	Thumbs:  []string{sdb.GetEpisodeImageUrl(ep)},
...
func writeTVShowNFO(baseDir string, series *series.Series, overrideImage bool) (err error) {
	...
	// for i, img := range series.Images[sdb.ImageTypeKeyFanArt] {
	...
	// 		}
	// 		switch img.KeyType {
	// 		case sdb.ImageTypeKeyPoster:
	// 			thumb.Aspect = "poster"
	// 			addImage("poster", img.FileName)
	// 		case sdb.ImageTypeKeySeries:
	// 			thumb.Aspect = "banner"
	// 			addImage("banner", img.FileName)
	// 		case sdb.ImageTypeKeySeason:
	// 			thumb.Aspect = "poster"
	// 			thumb.Type = "season"
	...
	// 			}
	// 		case sdb.ImageTypeKeySeasonWide:
	// 			thumb.Aspect = "banner"
	// 			thumb.Type = "season"
	// 			thumb.Season, _ = strconv.Atoi(img.SubKey)
	// 			if img.SubKey == "0" {
